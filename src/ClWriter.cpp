...
void SharedClWriter::writeDeclaration(std::string indent, TypeDumper *typeDumper, std::ostream &os) {
    ...
    if(PointerType *pointerType = dyn_cast<PointerType>(value->getType())) {
        ...
        Type::TypeID typeID = primitiveType->getTypeID();
        if (typeID == Type::ArrayTyID) {
        	ArrayType *type = cast<ArrayType>(primitiveType);
    	    int length = type->getNumElements();
    	    Type *elementType = type->getElementType();
//    	    if (length > 1)
//    	    	numElements *=length;
        	os << indent << "local " << typeDumper->dumpType(elementType, false) << " " << localValueInfo->name << "[" << numElements \
			<< "][" << length << "];\n";
        }
        else
