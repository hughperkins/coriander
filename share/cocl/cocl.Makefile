...
LLVM_COMPILE_FLAGS := `$(CLANG_HOME)/bin/llvm-config --cppflags --cxxflags | sed -e 's/ -fno-exceptions/ -fexceptions/g' -e 's/ -DNDEBUG//g' -e 's/ -std=c++0x/ -std=c++11/g' -r -e 's% -isysroot [^ ]+%%g'`
