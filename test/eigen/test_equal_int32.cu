...
{
  Tensor<int32_t, 3> in1(Eigen::array<Eigen::DenseIndex, 3>(72,53,97));
  Tensor<int32_t, 3> in2(Eigen::array<Eigen::DenseIndex, 3>(72,53,97));
  Tensor<bool, 3> out(Eigen::array<Eigen::DenseIndex, 3>(72,53,97));
  in1.setRandom();
  in2.setRandom();

  std::size_t in1_bytes = in1.size() * sizeof(int32_t);
  std::size_t in2_bytes = in2.size() * sizeof(int32_t);
  ...
  Eigen::TensorMap<Eigen::Tensor<int32_t, 3> > gpu_in1(d_in1, Eigen::array<Eigen::DenseIndex, 3>(72,53,97));
  Eigen::TensorMap<Eigen::Tensor<int32_t, 3> > gpu_in2(d_in2, Eigen::array<Eigen::DenseIndex, 3>(72,53,97));
